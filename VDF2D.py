...
class VDF:
    ...
    def BiMax(self, v_perp, v_par, v, n):
        par_thermal_speed = np.sqrt(2 * cst.k * self.T_par / cst.mp)
        perp_thermal_speed = np.sqrt(2 * cst.k * self.T_perp / cst.mp)

        bimax_value = self.n \
            / (par_thermal_speed * np.square(perp_thermal_speed))\
            * np.exp(-np.square(v_perp - v) / perp_thermal_speed)\
            * np.exp(-np.square(v_perp) / par_thermal_speed)
