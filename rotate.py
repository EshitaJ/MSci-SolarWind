...
# ecliptic frame: z= up out of plane, x=radially out from Sun,
# y=perpendicular to x&z via right hand rule


def euler_rodrigues_intrinsic_rotation(x, y, z,
                                       rot_axis='z', rot_angle=np.pi/4):
...
def frame_rotation(x, y, z, direction='B-ecliptic', sw_theta=np.pi/4, sw_phi=0):
    ...
    # x = parallel to B, z up out of ecliptic plane,
    # y = perp to both as in RH rule
...
if __name__ == '__main__':
    # x=np.array([5, 7, 8])
    # y=np.array([1, 12, 20])
    fr = frame_rotation(1, 0, 0, sw_theta=np.pi/4, sw_phi=np.pi/4)
    print(fr)
